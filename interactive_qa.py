...
def check_and_populate_database():
    ...
    try:
        ...
        if count == 0:
            print("📚 Database is empty. Let me populate it with workshop transcripts...")
...
def answer_with_llm(question):
    ...
    try:
        ...
        print(answer)
        print("\n" + "="*60)
        print(f"📊 Based on {context_info.get('num_chunks', 0)} chunks from workshops: {', '.join(context_info.get('workshops_used', []))}")
        print("="*60)
