...
def main(input_json_path):
    ...
    print(f"Input results file: {input_json_path}")
    ...
    try:
        ...
        print(f"Successfully loaded {len(results_data)} records from {input_json_path}")
    ...
    # --- Run LLM Judge --- #
    print(f"\nRunning LLM Judge ({JUDGE_MODEL}) on {len(results_data)} records...")
    judge_results = []
    for record in results_data:
        question = record.get('question')
        response = record.get('response')
        verdict, reason = get_llm_judge_verdict(client, question, response)
        judge_results.append({'question': question, 'response': response, 'llm_judge_verdict': verdict, 'llm_judge_reason': reason})
    ...
    output_path = os.path.join(os.path.dirname(input_json_path), output_filename)
    
    try:
        with open(output_path, 'w', encoding='utf-8') as f:
            json.dump(judge_results, f, indent=4) # Use indent for readability
    ...
    successful_judgements = sum(1 for result in judge_results if result['llm_judge_verdict'] is not None)
    print(f"Successfully judged {successful_judgements} out of {len(judge_results)} records.")
    ...
    pass_rate, num_judged = calculate_pass_rate(pd.DataFrame(judge_results), verdict_col='llm_judge_verdict') 
    ...
    # Note: Latency and token stats are not applicable for JSON input without these fields
    ...
    if baseline_metrics:
        ...
        print(f"Evaluated {len(judge_results)} records, successfully judged {num_judged}.")
...
def print_comparison_report(current_metrics, baseline_metrics):
    ...
    # --- Latency ---
    if 'latency_seconds' in current_metrics:
        print("\n--- Latency (seconds) --- ")
    ...
    cr_avg_tok = current_metrics.get('tokens_total', {}).get('average')
    ...
    cr_avg_cost = current_metrics.get('estimated_cost_usd', {}).get('average')
...
if __name__ == "__main__":
    ...
    parser.add_argument("input_json", help="Path to the JSON file containing the system results (question, response, etc.)")
